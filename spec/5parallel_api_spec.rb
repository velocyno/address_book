...
describe "Test in Parallel" do
  ...
  it 'signs up' do
    ...
    expect(Test::Site.new.logged_in?(user)).to eq true
  end

  it 'login' do
    Test::Site.new.create_user(user)
  ...
  it 'logout' do
    Test::Site.new.log_in_user(user)

    Home.visit.sign_out_user

    expect(Test::Site.new.logged_in?(user)).to eq false
  end
  ...
  it 'creates address' do
    ...
    expect(Test::Site.new.address?(address)).to eq true
  end

  it 'shows' do
    Test::Site.new.log_in_user(user)
    Test::Site.new.create_address(address)
  ...
  it 'lists' do
    Test::Site.new.log_in_user(user)
    Test::Site.new.create_address(address)
    address2 = Test::Site.new.create_address
  ...
  it 'edits' do
    ...
    expect(Test::Site.new.address?(edited_address)).to eq true
  end
  ...
  it 'deletes address' do
    ...
    expect(Test::Site.new.address?(address)).to eq false
  end
